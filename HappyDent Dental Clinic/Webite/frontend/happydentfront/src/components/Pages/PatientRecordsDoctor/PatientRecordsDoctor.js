...
const PatientRecordDoctor = ({ setToast }) => {
  // ---------------- DATA ----------------
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        header: "Actions",
        cell: ({ row }) => (
          <button onClick={() => openUpdate(row.original)} style={editBtnStyle}>
            Edit
          </button>
        ),
      },
...
const container = { padding: "30px 50px", backgroundColor: "#f7fafc", minHeight: "100vh" };
